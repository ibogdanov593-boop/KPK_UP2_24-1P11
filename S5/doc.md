...
| `manager` | Нет | str | длина ≤ 100 | `''` |
| `building` | Нет | str | длина ≤ 50 | `''` |
| `is_active` | Нет | bool | True/False | `True` |

**Уникальные комбинации параметров:**  
- `name` (название отделения уникально)
...
`True` — если отделение удалено, иначе `False` (в теле ответа поле `deleted`)
...
| `manager` | str | Поиск по заведующему (содержит подстроку) |
| `building` | str | Поиск по корпусу (содержит подстроку) |
